...
class UnsteadyAirfoil:
	...
	def solve_for_process(self,
		...
		for dt_i, (plate_angle, flap_angle, inflow) in enumerate(zip(plate_angles, flap_angles, inflows)):
			...
			self.geometry.displace_vortices(velocities=np.c_[x_vel, y_vel]+background_flow, time_step=dt)
			self.bound_circulation["plate"][dt_i] = np.sum(bound_circulation[:self.plate_res])
			self.bound_circulation["flap"][dt_i] = np.sum(bound_circulation[self.plate_res:])
			if (dt_i)%20 == 0:
				print("iteration: ", dt_i, "bound circulation: ", np.sum(bound_circulation[:-1]))
		return (self.bound_circulation,
